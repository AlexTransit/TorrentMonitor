...
class kinozalme
{
    ...
    public static function work($titlearray, $array, $id, $tracker, $name, $torrent_id, $timestamp, $hash, $auto_update, &$return)
    {
		...
		if (isset($array[1]))
		{
			...
			if ( ! empty($array[1]))
			{
				...
				$date = kinozalme::dateStringToNum($array[1]);
				$date_str = kinozalme::dateNumToString($array[1]);
	...
	public static function getRequestParams($params)
	{
		...
		{
			$header = array();
			foreach (array('Host' => 'kinozal.me', 'Content-length' => strlen(kinozalme::$sess_cookie)) as $k => $v)
				$header[] = $k.': '.$v."\r\n";
